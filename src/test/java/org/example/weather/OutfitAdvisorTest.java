...
public class OutfitAdvisorTest {
    ...
    @Mock // weatherService = Mockito.mock(WeatherService.class); esto lo necesitamos hacer en cada
    // test, pero con @Mock nos ahorramos el escribirlo en cada test
    WeatherService weatherService;

    @InjectMocks
    OutfitAdvisor advisor;

    //    @BeforeEach otra opción es tener un beforeEach, antes de cada test se ejecutará el mockito
    // de weatherService
    //    void setUp(){
    ...
    void freezingTemps() {
        ...
        //        para crear un objeto advisor, necesitamos una referencia de weather service y esto es
        // la dependency inyection, se hace a través del konstruktor
    ...
    void defaultAdviceWhenNoTempData() {
        Mockito.when(weatherService.getTemperature())
                .thenThrow(
                        new IllegalStateException()); // configuración para atrapar la excepción cuando no hay
        // una temperatura
